...
let add_vars_to_tbl t var class_id tbl var_count =
  match var.id with
...
let rec add_vars_to_tbl_rec t vars class_id tbl var_count = 
  match vars with
  | [] -> ();
  | (f::fs) -> 
      add_vars_to_tbl t f class_id tbl var_count;
      add_vars_to_tbl_rec t fs class_id tbl var_count;;
...
let rec add_func_elems_to_tbl elem tbls var_count ft oc=
  match elem with 
  | Asigna a -> process_asignacion a.izq a.der tbls oc; 
  | CondIf cif -> process_condition cif.cond tbls oc; process_block cif.true_block tbls var_count ft oc; process_block cif.false_block tbls var_count ft oc;
  | Escritura e -> process_print_rec e tbls oc;
  | EVar evar -> add_vars_to_tbl_rec evar.tipo evar.vars evar.id_class tbls.function_tbl var_count;
  | ForLoop floop -> process_for_loop floop tbls var_count ft oc;
  | WhileLoop wloop -> process_condition wloop.cond tbls oc; process_block wloop.bloque tbls var_count ft oc;
...
and add_func_elems_to_tbl_rec bloque tbls var_count ft oc=
  match bloque with
  | [] -> ()
  | (f::fs) -> 
      add_func_elems_to_tbl f tbls var_count ft oc;
      add_func_elems_to_tbl_rec fs tbls var_count ft oc
(* Process blocks for conditions and loops *)
and process_block bloque tbl var_count ft oc=
  add_func_elems_to_tbl_rec bloque tbl var_count ft oc
and process_for_loop floop tbl var_count ft oc= 
  variableLookup floop.init tbl;
  assert_equalCS CsBool (process_expression floop.cond tbl oc);
  add_func_elems_to_tbl (Asigna floop.post) tbl var_count ft oc;
  process_block floop.bloque tbl var_count ft oc;;


(* Match the element of a class to a function and add the function elements to tbl *)
let add_inner_fucs_of_class elem class_tbl tbl var_count oc=
  match elem, class_tbl with
  | Fun f, ClaseT ct -> add_func_elems_to_tbl_rec f.fbloque { function_tbl=(getFunctionTblInClass f.fname class_tbl); class_tbl=ClassTbl (getClaseTbl class_tbl); global_tbl=tbl} var_count f.tipo oc; ();
  | CVar cv, ClaseT ct -> ();
  | _, _ -> assert false;;

(* Iterate through the class to check on functions *)
let rec add_inner_fucs_of_class_rec bloque class_tbl tbl var_count oc = 
  match bloque with
  | [] -> ();
  | (f::fs) -> 
      add_inner_fucs_of_class f class_tbl tbl var_count oc;
      add_inner_fucs_of_class_rec fs class_tbl tbl var_count oc;;

(* Check elem to match class of function, then add all variables in functions to table*)
let add_inner_func_to_tbl elem tbl var_count oc = 
  match elem with
  | Func f -> add_func_elems_to_tbl_rec f.fbloque { function_tbl=(getFunctionTbl f.fname tbl); class_tbl=Nil; global_tbl=tbl} var_count f.tipo oc;
  | Clase c -> add_inner_fucs_of_class_rec c.bloque (Hashtbl.find tbl c.name) tbl var_count oc;;

(* Processing a single element of each class *)
let add_class_att_to_table_inner elem class_tbl var_count = 
  match elem, class_tbl with
  | Fun f, ClaseT ctbl -> add_func class_tbl f.fname {name=f.fname; tipo=f.tipo; variables=Hashtbl.create 0}; [];
  | CVar cv, ClaseT ctbl -> add_vars_to_tbl_rec cv.tipo cv.vars cv.id_class ctbl.vars var_count; [];; (*If type is class, validate class type *)

(* Iterating through all the class variables and funcs *)
let rec add_class_att_to_table bloque class_tbl var_count = 
  match bloque with 
  | [] -> class_tbl;
  | (f::fs) -> 
      add_class_att_to_table_inner f class_tbl var_count;
      add_class_att_to_table fs class_tbl var_count;;

(* Processes single element of high order *)
let add_upper_prog_to_table elem table var_count = 
  match elem with
  | Func f -> add_high_level_element table elem
  | Clase c -> let class_tbl = add_high_level_element table elem in add_class_att_to_table c.bloque class_tbl var_count;;

(* Iterating through all the elemnts in tree *)
let rec semantic_main tree table var_count oc = 
  match tree with 
  | Program [] -> table
  | Program (i :: j) ->
      add_upper_prog_to_table i table var_count; (* Adds high level funcs and classes to main table, including vars and funcs of class *)
      semantic_main (Program j) table var_count oc; (* Iterate *)
      add_inner_func_to_tbl i table var_count oc; (* Process functions of classes or funcs *)
      table;;
...
let initialize_count tbl =
  ...
  Hashtbl.add tbl BoolTy {count=0; base=4000};;
...
let semantic_start tree oc = 
  let main_table = Hashtbl.create 1234 in
    let var_count = Hashtbl.create 1234 in
      ...
      semantic_main tree main_table var_count oc;;
